...
class NewsService {
    ...
    getAllNews = (includeDrafts: boolean = false, page: number = 1, limit: number = 10, status?: string, lang?: string, state?: string): Promise<ApiResponse<NewsItem[]>> => {
        const endpoint = `/getallnews?page=${page}&limit=${limit}${includeDrafts ? '&includeDrafts=true' : ''}${status ? `&status=${status}` : ''}${lang ? `&lang=${lang}` : ''}${state ? `&state=${state}` : ''}`;
        return this.request(endpoint);
    };

    getNewsBySection = (section: string, includeDrafts: boolean = false, page: number = 1, limit: number = 10, status?: string, lang?: string, state?: string): Promise<ApiResponse<NewsItem[]>> => {
        const endpoint = `/getnewsbysection/${section}?page=${page}&limit=${limit}${includeDrafts ? '&includeDrafts=true' : ''}${status ? `&status=${status}` : ''}${lang ? `&lang=${lang}` : ''}${state ? `&state=${state}` : ''}`;
        return this.request(endpoint);
    };
