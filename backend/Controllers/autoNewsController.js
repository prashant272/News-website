...
const autoGenerateNews = async (req, res) => {
    try {
        ...
        // APPLY BRANDING
        const branded = ensureBranding(aiDataEn.title, createSlug(aiDataEn.title), "en");
        const finalTitle = branded.title;
        const finalSlug = branded.slug;

        const urlHash = generateUrlHash(url);

        const existing = await NewsArticle.findOne({ $or: [{ slug: finalSlug }, { urlHash: `${urlHash}-en` }] });
        ...
        const newPostEn = new NewsArticle({
            title: finalTitle,
            slug: finalSlug,
        ...
        await newPostEn.save();
        res.json({ success: true, msg: "News auto-generated and watermarked.", post: newPostEn });
    } catch (error) {
